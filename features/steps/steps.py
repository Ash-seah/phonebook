...
@when('I send a PUT request to "/" with name "{name}" and number "{number}"')
def step_when_put_record(context, name, number):
    context.response = requests.put(f"{BASE_URL}/", json={"name": name, "number": number})
